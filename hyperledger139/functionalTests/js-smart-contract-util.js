...
class SmartContractUtil {
    ...
    static async getConnectionProfile() {
        ...
        const connectionProfilePath = path.join(homedir, '.fabric-vscode', 'environments', 'MyAssignmentEnv', 'gateways', 'Org1 Gateway.json');
    ...
    static async submitTransaction(contractName, functionName, args, gateway) {
        ...
        let contract;
        if (contractName !== '') {
            contract = await network.getContract('MyAssetJpContract', contractName);
        } else {
            contract = await network.getContract('MyAssetJpContract');
        }
